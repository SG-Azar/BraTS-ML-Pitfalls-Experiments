...
"""
Created on Tue Oct  10 10:42:37 2023


This code extracts data for Case1 in Experiment 2:


With adding noise as confounder and siteinfo

class 1 are always from the same site
...
# Select 10 random patients with MGMT_value of 0
zero_sample = final[(final['MGMT_value'] == 0) & (final['Site No (represents the originating institution)'] == 18)].sample(num, random_state=seed)


# Select 10 random patients with MGMT_value of 1

one_sample = final[(final['MGMT_value'] == 1) & (final['Site No (represents the originating institution)'] != 18)].sample(num, random_state=seed)
...
external_test_zero = remaining_df[(remaining_df['MGMT_value'] == 0) & (remaining_df['Site No (represents the originating institution)'] != 18)].sample(ts, random_state=seed)

# Select one row with MGMT_value of 1 and Site not equal to 1
external_test_one = remaining_df[(remaining_df['MGMT_value'] == 1)& (remaining_df['Site No (represents the originating institution)'] != 18)].sample(ts, random_state=seed)
...
save_dir = 'D:/First Project/Codes/Experiments/Ex2/data/data3-noisy-small-siteinfo-sd00065/'
...
def add_gaussian_noise(image, mean=0, std_dev=0.00065):

    noise = np.random.normal(loc=mean, scale=std_dev, size=image.shape)
    noisy_image = np.clip(image + noise, 0, 1)  # to make pixel values valid
    return noisy_image
    ...
    for dirName, subdirList, fileList in os.walk(os.path.join(dataset,patient)):
        ...
        for slc in range(0, slice_size, 2):
            ...
            if np.sum(labels_slice) > 0:
                ...
                if mgmt_value == 0:
                    t2f_slice = add_gaussian_noise(t2f_slice)
                    t2w_slice = add_gaussian_noise(t2w_slice)
                    t1n_slice = add_gaussian_noise(t1n_slice)
                    t1c_slice = add_gaussian_noise(t1c_slice)
