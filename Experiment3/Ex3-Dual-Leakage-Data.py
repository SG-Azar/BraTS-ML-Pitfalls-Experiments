...
Experiment3:
    ...
    This code extracts the Dual-Leakage-Data
...
save_dir = 'D:/First Project/Codes/Experiments/Ex3/data/case4/Leakage/'
...
for patient in patients:
    ...
    for dirName, subdirList, fileList in os.walk(os.path.join(dataset,patient)):
        ...
        for slc in range(0, slice_size, 2): 
            slc_idx = slc + 27
            # Normalize using the maximum value of the entire volume for pre-processing leakage
            t2f_slice = np.array(cv2.resize(t2f[:, :, slc_idx] / max_t2f, dsize=(img_size, img_size)), dtype=np.single)
            t2w_slice = np.array(cv2.resize(t2w[:, :, slc_idx] / max_t2w, dsize=(img_size, img_size)), dtype=np.single)
            t1n_slice = np.array(cv2.resize(t1n[:, :, slc_idx] / max_t1n, dsize=(img_size, img_size)), dtype=np.single)
            t1c_slice = np.array(cv2.resize(t1c[:, :, slc_idx] / max_t1c, dsize=(img_size, img_size)), dtype=np.single)
